...
class Registry:
    ...
    def registry(self)->dict[str,Function]:
        tools_registry={}
        for tool in self.tools:
            func=Function(name=tool.name,description=tool.description,params=tool.params,function=tool.func)
            tools_registry.update({tool.name : func})
    ...
    def execute(self,name:str,input:dict,**kwargs)->ToolResult:
        ...
        try:
            ...
            print(e)
            content='Tool Execution Failed. Try again.'
        return ToolResult(name=name,content=content)
